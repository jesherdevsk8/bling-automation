...
if exit_code == 0:
  ...
  for file in jpg_files:
    ...
    try:
      tab_images = WebDriverWait(driver, 10).until(
      ...
      # print(f'\nNão deu certo!.... Salvando ID: {product_id} no arquivo de cache')
    ...
    click_to_upload = WebDriverWait(driver, 10).until(
      EC.element_to_be_clickable((By.CSS_SELECTOR, 'div.qq-upload-drop-area-dashed span'))
    )

    click_to_upload.click()

    upload_input = WebDriverWait(driver, 10).until(
      EC.presence_of_element_located((By.CLASS_NAME, 'qq-file-input'))
    )
    ...
    close_button = modal.find_element(By.CSS_SELECTOR, '.ui-dialog-titlebar-close')
    close_button.click()
    ...
    error_message_element = WebDriverWait(driver, 10).until(
      EC.presence_of_element_located((By.CSS_SELECTOR, 'div.alert-box.alert-box-warning h4'))
    )
    ...
    if error_message.strip() == 'Não foi possível salvar o produto':
      write_error_file(product_id)
    else:
      write_on_success_file(product_id)
      print('\nProduto id', product_id, 'salvo com sucesso!')
